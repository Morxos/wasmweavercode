...
def main():
    ...
    env = gym.make("gymnasium_env/WasmWeaverEnv-v0",
                   constraints=[ByteCodeSizeConstraint(0, 1000), FuelConstraint(0, 300)],
                   output_types=all_output_classes, post_processor_types=[],
                   forbidden_instruction_name_tokens=["load","store"],
                   reward_function=SimpleRewardFunction(f"{experiment_name}_samples",flag_reward=False, depth_reward=True, target_trace_length=100),
                   verbose=True)
