...
This flexible random program generator produces pseudo-random, correct WebAssembly code (and shared memory) that always runs to completion without exceptions, guided by a DRL policy.
...
- Basic setup of the tool and guides for the presented exemplary use cases
...
##### Memory Instructions
- [ ] memory.grow: Currently not needed in use cases, even when memory tracking is on.
- [-] memory.size: Currently always returns 1 (page), due to limited use cases. As a result, it's normally disabled.
...
- core: Contains most code for the raw generator, code corpus handling, code metric extraction, as well as the implemented code tiles and code tile factories (see core/instructions).
	...
	- Our generator is based on **code tiles** and **factories**. Where tiles are used for most normal instructions, factories generate code tiles based on the current state. (e.g., call <function> tile is only generated when a function is already defined). Abstract classes for both are in **core/tile.py**.
	- The implementations of the global state, which tracks stack, linear memory, (external/internal) functions, globals, tables, etc., can be found in state/<state.py,globals.py,functions.py, etc.>
	...
	- processor.py contains post-processors that are applied to the global state after program generation has finished. Currently, we implement the StackInspectorPostProcessor and the FlagReachabilityPostProcessor, which add and evaluate ;;INSPECT and ;;FLAG_N comments respectively.
	...
	- strategy.py implements DRL-independent strategies for code tile selection (e.g., a simple random strategy).
	- extractor.py is used to extract trigram information from a given code corpus. The input and output paths can be set at line 142 (path = ...) and line 143 (output_dir = ...).
...
- experiments: Contains code for training and evaluating our different scenarios:
	- training: Contains code for different experiments
	...
	- eval: Contains code for evaluation:
		...
		- models: Contains model definitions and tokens for OpenAI and self-hosted models:
			...
			- openai_models.py: Defines interfaces for OpenAI models
				- Before running the evaluation on one of those models, set the OPENAI_TOKEN in line 9
	...
	- wasmbench: Contains extracted opcode data from the WasmBench (https://github.com/sola-st/WasmBench) reference corpus. Also, the filtered WasmBench samples should be placed here.
...
#### Example Scenarios
The following scenarios demonstrate how to use the tool for various tasks, including code generation with high-level rewards and LLM benchmarking.
All samples are in the "experiments/training" folder.
...
##### Extract Dataset Metrics:
	- Run dataset_metric_extractor.py to extract simple dataset metrics. Before use, set the dataset folder name (folder_name, line 8)
...
#### Gymnasium Environment
	...
	- These are then converted to numeric gymnasium spaces with embedding modules, located under drl/embedder/... The IDs found in drl/embedder/tiles.py->get_id() (code tile IDs) can also be used to decode the available actions from the action mask.
